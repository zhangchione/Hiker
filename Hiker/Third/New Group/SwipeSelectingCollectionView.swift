...
import UIKit
@available(iOS 10.0, *)
public protocol SwipeSelectingDelegate: class {
...
public class SwipeSelectingCollectionView: UICollectionView {
    ...
    public weak var swipteDelegate: SwipeSelectingDelegate?
    ...
    var isOpenChoice = false {
        didSet {
            panSelectingGestureRecognizer.isEnabled = isOpenChoice
            allowsMultipleSelection = isOpenChoice
            longGestureRecognizer.isEnabled = !isOpenChoice
            swipteDelegate?.selecteStatusChange(self, with: isOpenChoice)
        }
    }
    ...
    lazy private var longGestureRecognizer: UILongPressGestureRecognizer = {
        let gestureRecognizer = UILongPressGestureRecognizer(
            target: self,
            action: #selector(SwipeSelectingCollectionView.didlongGestureRecognizer(gestureRecognizer:)))
    ...
    @objc private func didlongGestureRecognizer(gestureRecognizer: UIGestureRecognizer) {
        isOpenChoice = true
        ...
        didPanSelectingGestureRecognizerChange(gestureRecognizer: gestureRecognizer)
    }

	@objc private func didPanSelectingGestureRecognizerChange(gestureRecognizer: UIGestureRecognizer) {
		let point = gestureRecognizer.location(in: self)
		...
		case .changed:
            guard isOpenChoice else { return }
    ...
    func deSelectAll() {
        guard let selectedIndexs = indexPathsForSelectedItems else { return }
	...
	func setSelection(_ selected: Bool, indexPath: IndexPath) {
		switch selected {
		case true:
