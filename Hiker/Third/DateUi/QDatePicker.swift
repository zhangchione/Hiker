...
class QDatePicker: UIView, UIGestureRecognizerDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    typealias DidSelectedDate = (_ date: Date) -> Void ///< 定义确认回调
    ...
    open var themeColor: UIColor? = UIColor.hex(hex: 0x00B3C4) { ///< 主题颜色样式
        didSet {
    ...
    open func show() {
        ...
        if animationStyle == .styleDefault {
            ...
            UIView.animate(withDuration: duraton, delay: 0, usingSpringWithDamping: 0.9, initialSpringVelocity: 0, options: UIView.AnimationOptions([.curveEaseInOut, .beginFromCurrentState, .layoutSubviews]), animations: {
                self.backgroundColor = UIColor.black.withAlphaComponent(0.4)
                self.sureView.transform = CGAffineTransform(translationX: 0, y: y)
            })
            UIView.animate(withDuration: duraton, delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0, options: UIView.AnimationOptions([.curveEaseInOut, .beginFromCurrentState, .layoutSubviews]), animations: {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        if pickerStyle == .datePicker {
            ...
            if let minLimitDate = minLimitDate, intentDate.compare(minLimitDate) == ComparisonResult.orderedAscending {
                intentDate = minLimitDate
                resetDateShow(date: minLimitDate)
            }
            if let maxLimitDate = maxLimitDate, intentDate.compare(maxLimitDate) == ComparisonResult.orderedDescending {
