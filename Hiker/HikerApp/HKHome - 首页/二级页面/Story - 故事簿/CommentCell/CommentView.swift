...
//        bgView.addGestureRecognizer(dismissKetboardTap)
    ...
    func configUI(){
        ...
        self.titleLab = UILabel.tk_createLabel(text: "添加评论哦", textColor:  kBlack, font: BoldFontSize(24), supView: self.bgView, closure: { (make) in
    ...
    @objc func start(){
        ...
        comment.time = time
        var user = User()
        user.headPic = getHeadPic()!
        user.id = getUserId()!
        user.nickName = getNickName()!
        comment.user = user
        delegate?.passBookName(with:comment)
        self.textField.text = ""
        self.hiddenAddView()
    }
}
